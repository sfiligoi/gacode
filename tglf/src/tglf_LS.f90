      ...
      if (filter_in.lt.0.0) then
        ...
        do imax = 1, MIN(nb_of_unstable_modes,maxmodes)
          ...
          if (jmax(imax) > 0) then
            ...
            ! ------------------------------------------------------------------------
            ! PARITY CHECK BASED ON HERMITE BASIS DECOMPOSITION
            ! ------------------------------------------------------------------------
            ! Each eigenfunction is expanded on a Hermite basis with known parity:
            !   - odd indices j = 1, 3, 5, ... : even-parity basis functions
            !   - even indices j = 2, 4, 6, ... : odd-parity basis functions
            !
            ! The electrostatic potential phi is reconstructed using the imaginary part
            ! of field_weight(j), which reflects the mode structure for Re[phi]
            !
            ! We estimate the mode's parity by computing:
            ...
            do j = 1, nbasis
              total_weight = total_weight + AIMAG(field_weight_QL_out(1, j))**2
              if (MOD(j, 2) == 0) then
                odd_weight = odd_weight + AIMAG(field_weight_QL_out(1, j))**2
              end if
            end do
        ...
        ! ------------------------------------------------------------------------
        ! Clear outputs and re-sort eigenvalues
        ! ------------------------------------------------------------------------
        do j1 = 1, maxmodes
          jmax(j1) = 0
          gamma_out(j1) = 0.0
