...
dnl
dnl Version: $Id: nim_hdf5.m4 2792 2008-11-02 13:39:32Z cary $
dnl
...
dnl
dnl However, SZIP_LIB is needed, as it might vanish if not found.
dnl
...
dnl ######################################################################
dnl
dnl NERSC stuffs everything into HDF5 (too general for my tastes)
if test -n "$HDF5"; then 
      SAVE_HDF5_LIBS="$HDF5"
      SAVE_HDF5_INC="$HDF5"
fi
dnl NCCS puts libs and includes into one var for Fortran
if test -n "$HDF5_FLIB"; then 
	  SAVE_HDF5_LIBS="$HDF5_FLIB" 
	  SAVE_HDF5_INC="$HDF5_FLIB"
fi

if test -n "$HDF5_LIBS"; then SAVE_HDF5_LIBS="$HDF5_LIBS"; fi
if test -n "$HDF5_INC"; then SAVE_HDF5_INC="$HDF5_INC"; fi
if test -n "$HDF5_LIBDIR"; then SAVE_HDF5_LIBDIR="$HDF5_LIBDIR"; fi
if test -n "$HDF5_INCDIR"; then SAVE_HDF5_INCDIR="$HDF5_INCDIR"; fi
if test -n "$HDF5_DIR"; then SAVE_HDF5_DIR="$HDF5_DIR"; fi
...
dnl JRC, 22May06: Commenting this out, as I do not understand the reason.
dnl Wouldn't the default to be to disable?  I.e., build with if available?
dnl AC_ARG_ENABLE(hdf5, AC_HELP_STRING([--enable-hdf5], [use HDF5]),
dnl      	[case "${enableval}" in
dnl        		yes | no) ac_cv_have_hdf5="${enableval}" ;;
dnl 		*) AC_MSG_ERROR(bad value ${enableval} for --enable-HDF5) ;;
dnl      	esac],
dnl 	[ac_cv_have_hdf5=yes])

dnl Most AC_ARG_WITH do not use AC_HELP_STRING, as help not needed.
AC_ARG_WITH(hdf5-dir, [  --with-hdf5-dir=<hdf5 installation directory>],
	HDF5_DIR="$withval", HDF5_DIR="")

dnl JRC, 22May06: With above gone, this should not be needed.
dnl In any case, no harm in finding.  If not found, configure.ac
dnl can decide what to do.
dnl if test -n "$MUST_HAVE_H5DIFF"; then
dnl   if test $ac_cv_have_hdf5 = no; then
dnl     AC_MSG_WARN(This project must have HDF5, so you cannot switch it off. Turning HDF5 back on.)
dnl     ac_cv_have_hdf5=yes
dnl   fi
dnl fi
...
# Store for later use
  HDF5_SERINCPATH=$HOME/$UNIXFLAVOR/hdf5/include:/opt/hdf5/include:/usr/local/hdf5/include:/loc/hdf5/include:/usr/common/usg/hdf5_64/default/serial/include:/usr/common/usg/hdf5/64/default/serial/include
  HDF5_PARINCPATH=$HOME/$UNIXFLAVOR/hdf5mpi/include:/opt/hdf5/include:/usr/local/hdf5mpi/include:/loc/hdf5mpi/include:/usr/common/usg/hdf5_64/default/parallel/include:/usr/common/usg/hdf5/64/default/parallel/include
...
if test $ac_cv_have_hdf5 = yes; then
  ...
  TX_PATH_FILE(LIBHDF5_A, libhdf5.a, "", $HDF5_LIBPATH)
  if test -z "$LIBHDF5_A"; then
    AC_MSG_WARN(libhdf5.a not found.  Use --with-hdf5-libdir=)
    ac_cv_have_hdf5=no
  else
...
dnl   ac_use_fortran_hdf5=yet        # Perform fortan hdf5 tests
dnl
dnl ######################################################################
if test $ac_cv_have_hdf5 = "yes"; then
	if test "$FCBASE" != none && test -n "$FCBASE" && test $ac_use_fortran_hdf5 = yes; then
	  dnl First check to see if command line gives a fortran library
	  if test -n "$SAVE_HDF5_LIBS"; then 
	     hdf5_fortran=`echo $SAVE_HDF5_LIBS 2>/dev/null | grep hdf5_fortran`
	     if test -n "$hdf5_fortran"; then
		  ac_cv_wrap_hdf5f90=no
	     else
		  ac_cv_wrap_hdf5f90=yes
	     fi
	  else
		if test -f $HDF5_LIBDIR/libhdf5_fortran.a; then
		    dnl test to make sure hdf5_fortran exist
		    hdf5_f90_full=`grep 'Fortran Compiler' $HDF5_LIBDIR/libhdf5_fortran.settings | cut -f2 -d:`
		    hdf5_f90=`basename $hdf5_f90_full`
		    if test "$FCBASE" = $hdf5_f90; then
			dnl This is to handle the Absoft weirdness
			HDF5_INC="$HDF_INC $FC_INCLUDE_FLAG$HDF5_LIBDIR/includes $SZIP_INC"
			HDF5_LIBS="$HDF5_LIBS -lhdf5_fortran $SZIP_LIBS"
			AC_SUBST(HDF5_LIBS)
			AC_SUBST(HDF5_INC)
			ac_cv_wrap_hdf5f90=no
		    else
			AC_MSG_WARN(Fortran compiler used to compile hdf5 bindings --
			$hdf5_f90 not the same as your fortran compiler -- $FC.  
			Try building wrappers by hand.)
			ac_cv_wrap_hdf5f90=yes
		    fi
		  else 
		    AC_MSG_WARN(hdf5_fortran.a not found in $HDF5_LIBDIR. Try to make wrappers.)
		    ac_cv_wrap_hdf5f90=yes
		  fi
	  fi
	fi
...
AC_ARG_WITH(szip-dir, [  --with-szip-dir=<szip installation directory> ],
	SZIP_DIR="$withval", SZIP_DIR="")

dnl ######################################################################
dnl
dnl Find szip includes - look first in szip area, then in other locations.
dnl
dnl ######################################################################

NEEDSZIP=`grep H5_HAVE_FILTER_SZIP $HDF5_INCDIR/H5pubconf.h | grep define`
if test -n "$NEEDSZIP"; then
  AC_ARG_WITH(szip-incdir, [  --with-szip-incdir=<szip include directory>],
	SZIP_INCDIR="$withval", )
  if test -n "$SZIP_INCDIR"; then
    SZIP_INCPATH=$SZIP_INCDIR
  elif test -n "$SZIP_DIR"; then
    SZIP_INCPATH=$SZIP_DIR/include
  else
    SZIP_INCPATH=/usr:$HOME/szip/include:/usr/local/szip/include:/loc/szip/include:/usr/common/usg/szip/default/include:/usr/common/usg/szip/2.0/include
  fi
dnl  AC_PATH_PROGS(SZLIB_H, szlib.h, "", $SZIP_INCPATH)
  TX_PATH_FILE(SZLIB_H, szlib.h, "", $SZIP_INCPATH)
  if test -z "$SZLIB_H"; then
    AC_MSG_WARN(szlib.h not found in $SZIP_INCPATH.  Use --with-szip-incdir=)
    ac_cv_have_szip=no
  else
    SZIP_INCDIR=`dirname $SZLIB_H`
    SZIP_INC=-I$SZIP_INCDIR
    SZIP_DIR=`dirname $SZIP_INCDIR`
    ac_cv_have_szip=yes
  fi
else
  ac_cv_have_szip=no
fi
AC_SUBST(SZIP_INCDIR)
AC_SUBST(SZIP_INC)
...
AC_ARG_WITH(szip-libdir, [  --with-szip-libdir=<szip lib directory>],
	SZIP_LIBDIR="$withval",SZIP_LIBDIR="")
if test $ac_cv_have_szip = yes; then
  if test -n "$SZIP_LIBDIR"; then
    SZIP_LIBPATH=$SZIP_LIBDIR
  else
    SZIP_LIBPATH=$SZIP_DIR/lib
  fi
  # echo SZIP_LIBPATH=$SZIP_LIBPATH
  # echo SZIP_LIBDIR=$SZIP_LIBDIR
  TX_PATH_FILE(LIBSZIP_A, libsz.a,"", $SZIP_LIBPATH)
  if test -z "$LIBSZIP_A"; then
    AC_MSG_WARN(libsz.a not found.  Use --with-szip-libdir=)
    ac_cv_have_szip=no
  else
    SZIP_LIB=-lsz
    SZIP_LIBDIR=`dirname $LIBSZIP_A`
dnl JRC: commented out the RPATH_FLAG?  Restoring as needed for Linux.
    dnl SZIP_LIBS="-L$SZIP_LIBDIR -lsz"
    SZIP_LIBS="-L$SZIP_LIBDIR "'$(RPATH_FLAG)'"$SZIP_LIBDIR -lsz"
  fi
fi
AC_SUBST(SZIP_LIB)
...
dnl ######################################################################
if test -n "$SAVE_HDF5_LIBS"; then
      HDF5_LIBS="$SAVE_HDF5_LIBS"
      ac_cv_have_hdf5=yes
fi 
if test -n "$SAVE_HDF5_INC"; then
      HDF5_INC="$SAVE_HDF5_INC"
      ac_cv_have_hdf5=yes
fi
...
if test -n "$config_summary_file"; then
  if test $ac_cv_have_hdf5 = yes; then
    ...
    echo "  SZIP_INC:    $SZIP_INC"           >> $config_summary_file
    echo "  SZIP_LIBDIR: $SZIP_LIBDIR"        >> $config_summary_file
    echo "  SZIP_LIBS:   $SZIP_LIBS"          >> $config_summary_file
  fi
fi
