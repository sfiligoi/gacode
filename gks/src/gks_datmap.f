...
c
c      if (use_xneo_m.eq.-1) then
c        do j=1,nj_d
c          chiineo_exp(j-1)=xkineo_d(j)/gradrhosq_exp(j-1)
c        enddo
c      endif
c      if (use_xneo_m.le.1 .and. use_xneo_m.ge.0) then
c        do j=1,nj_d
c          chiineo_exp(j-1)=xkineo_d(j)/ni_exp(j-1)/1.D19/
c     >                     (bscale*ascale**(.5D0))
c     >                     /gradrhosq_exp(j-1)
c          chieneo_exp(j-1)=chiineo_exp(j-1)/42.D0/
c     >                     dsqrt(amassgas_exp)
c        enddo
c      endif
