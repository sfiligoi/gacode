...
phi2 = (sim.kxky_phi[0,:,0,:,:]**2+sim.kxky_phi[1,:,0,:,:]**2)/sim.rho**2
...
print('INFO: (cgyro_distill).py) Wrote out.cgyro.phi2kxky = <|phi_k|^2>/rho^2_*D')
print('INFO: (cgyro_distill).py) Wrote out.cgyro.phi0kx   = <Re(phi_0)>/rho_*D, Im(phi_0)>/rho_{D*}')
print('INFO: (cgyro_distill).py) All field normalized to rho_{*D}')
print('INFO: (cgyro_distill).py) '+header)
