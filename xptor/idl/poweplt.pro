...
; Procedure to plot powe_m, powe_exp profiles
...
ncdf_varget,id,ncdf_varid(id,'powe_exp'),powex
ncdf_varget,id,ncdf_varid(id,'powe_m'),powem
; printf,-1,f
...
plot, x(*),powex(*),title='Electron Power',xtitle='rho',ytitle='T (kev)',thick=2,yrange=[0,1],color=dev.cnames.red
usersym,cos(a),sin(a)
oplot,x(*),powem(1:nr),thick=2,color=dev.cnames.blue
xyouts, .85, 0.95, names(0),charsize=1.50,color=dev.cnames.blue
xyouts, .85, 0.90, names(1),charsize=1.50,color=dev.cnames.red
print,'         rho           Powe_exp         Powe_m'
for m=1,nr do print, x(m), powex(m), powem(m)
...
a=findgen(17) * (!pi*2/16.)
usersym,cos(a),sin(a),/fill
plot, x(*),powex(*),title='Electron Power vs rho',xtitle='rho',ytitle='T (kev)',thick=2,yrange=[0,1],color=dev.cnames.red
usersym,cos(a),sin(a)
oplot,x(*),powem(1:nr),thick=2,color=dev.cnames.blue
xyouts, .85, 0.95, names(0),charsize=1.50,color=dev.cnames.blue
xyouts, .85, 0.90, names(1),charsize=1.50,color=dev.cnames.red
