...
c
     ...
     &         (1.D0 + 1.46D0*dsqrt(delt))
         vthe = dsqrt(2.D0*te_m(k)*1.6D-9/xmasse)
         xlam=24.D0-dlog( dsqrt(ene)/(1.D3*te_m(k)) )
         xnue=(4.D0/3.D0)*dsqrt(2.D0*pi)*ene*zf*
     &        charg4*xlam / dsqrt(xmasse*(te_m(k)*1.6D-9)**3.D0)
...
c
      do k=1,ngrid
         lnlam=24.D0-dlog(sqrt(1.D13*ne_m(k))/1.D3/te_m(k))
         lnlam=dmax1(lnlam,1.D0)
         eta(k) = 0.5D0*1.03D-2*zeff_exp(k)*lnlam/(1000.D0*te_m(k))**1.5D0
...
c
      do k=1,ngrid
c         Pohpro(k)=eta(k)*cur(k)**2
         Pohpro(k)=eta(k)*(curden_exp(k)/1.D4)**2
