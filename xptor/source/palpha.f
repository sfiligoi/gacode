...
c
     ...
     &       rtrit, cst1, theta, f, Wwcrit, dtrate
c
...
c
      ...
      do k=1,ngrid
        denIO = 10.D0*dinpro(k)
        nine  = dinpro(k)/denpro(k)
        Tii   = max(1.D-4,Tipro(k))
        npne  = dznpro(k)/denpro(k)
     ...
     &         dexp(-3.D0 * f)
        ...
        Wwcrit=58.3333D0/(Tepro(k)*((nine/2.D0+
...
c       write(*,50) k,rho(k),Tipro(k),dinpro(k)*1.D20,
c    &              sigv,Pfuspro(k)
      enddo
c
...
c
         if(idt.eq.1) then
           ...
           fusrate=(dinpro(k)*1.D20/2.D0)**2.D0*sigv
         endif
...
c
      do k=1,ngrid
         zTi=max(1.D-4,Tipro(k))
         sigv  = dtrate(zTi)*1.D-6   ! Bosch-Hale sigma-v
c         sigvv = sigvth(zTi)         ! TRANSP sigma-v
c         write(*,50) k, rho(k), sigv, sigvv
         if(idt.eq.1) then
           fusrate=(nm2_exp(k)*1.D19)*(nm3_exp(k)*1.D19)*sigv
         else
           fusrate=(dinpro(k)*1.D20/2.D0)**2.D0*sigv
...
c
      ...
      dtrate = C1*theta*DSQRT(xsi/(mrcsq*ti**3)) * DEXP(-3.D0*xsi)
      return
c
      end
