c@callglf2d.f
c 02-may-02 added fast ion pressure pfast_exp
c 18-aug-01 included diagnostic arrays gamma_k_j, freq_k_j, chie_k_j,
c           chii_k_j, chie_etg_j, chie_itg_j
c 05-mar-01 Kinsey, added iglf for new version of model
c 29-aug-00 Kinsey, added ave_ve for 3pt smoothing of ve (0 for none)
c 23-aug-00 Kinsey, added xkymax_m, xkymax2_m
c 20-dec-99 Kinsey, added sfactor and vprime for Gary's calc of gfac
c           igfac added to glf.m as a switch
...
************************************************************************
     ...
     >  xkymax_m,      !0:jmaxm leading mode ky-rhos
     >  xkymax2_m      !0:jmaxm 2nd leading mode ky-rhos
     > )
c************************************************************************
...
c external arrays
     ...
     >  i_delay, j, jin, jout, jm, irotstab,
     >  jpt, jptf, jptt, jj, ii, ik, bt_flag, leigen, nroot
      real*8 alpha_e, alpha_d, x_alpha, xalpha,
     ...
     >  bteff_exp(0:jmaxm)
      real*8 alpha_exp(0:jmaxm), pfast_exp(0:jmaxm),
     >  elong_exp(0:jmaxm),
...
c working arrays and variables
      ...
      real*8 ve(0:jmaxm),vpar(0:jmaxm),ve_avg(0:jmaxm)
      real*8 zpti_low, zpni_low, csda_low, rhosda_low, ve_low
      real*8 alpha_neo_low, alpha_neo_mid, 
     >       pgeo_local_low, pgeo_local_mid, egamma_avg(0:jmaxm),
     >       pgeo_local_high, zpti_high, zpni_high,
     >       csda_high, rhosda_high, alpha_neo_high, ve_high
...
c     real*8 diff_k_j(20,0:jmaxm), chie_e_k_j(20,0:jmaxm),
c    >       chie_etg_j(20,0:jmaxm), chie_itg_j(20,0:jmaxm)
...
c       diff_k_j(ik,j)    = 0.D0
c       chie_k_j(ik,j)    = 0.D0
c       chie_etg_j(ik,j)  = 0.D0
c       chie_itg_j(ik,j)  = 0.D0
...
c**********************************************************************
c     glf23 model parameters
c     Note: default settings for retuned GLF23 v1.61 (iglf=3)
c           iglf=0 for original model, iglf=2 for renormed model
c
ck      write(6,*)  'jmm=',jmm,'going in'
c
      eigen_gf = leigen
      nroot_gf=nroot  ! 8 for pure plasma, 12 for full impurity dynamics
      iflagin_gf(1)=0
      iflagin_gf(2)=1
      iflagin_gf(3)=1
      iflagin_gf(4)=0
...
c
      cbetae=1.D-6
c     cbetae=1.D0 !  full electromagetic
...
c       ialphastab=1
c 
c       EXB shear stabilization
c       (alpha_e_gf=1.0 for full ExB shear)
        alpha_e_gf=alpha_e
c 
...
c
c Transport flags:
c itport_pt(1)=1 ; density transport on
c itport_pt(2:3)=1; electron and ion heat transport on
...
c Notes: neoclassical drag built into vphi and vtheta transport equations...
c       -> consult G.M. Staebler
c       If only vphi_exp is available itport_pt(4)=0
c
c       grid-centering in computing EXB shear spans jm-jptf to jm+jpt
c
c      turn on high-k eta-e modes
c       xparam_gf(10)=1.D0
        if (xparam_gf(10).eq.0) xparam_gf(10)=1.D0
c
...
c        xparam_gf(15)=0.15D0   ! trapped ptcle fraction enhancement
c
...
c   local rho_star
c
c use effective B-field if bt_flag > 0
c
...
c   use effective B-field if bt_flag >= 2
c
       if (bt_flag .ge. 2) then
         cgyrobohm_m(jm)=1.D-4*
     >     9.79D5*(tem*1.D3)**.5D0/(arho_exp*100.D0)
     >     *(1.02D2*(tem*1.D3)**.5D0/bteff_exp(j)/1.D4)**2
     >     *amassgas_exp**.5D0
       else
         cgyrobohm_m(jm)=1.D-4*
     >     9.79D5*(tem*1.D3)**.5D0/(arho_exp*100.D0)
     >     *(1.02D2*(tem*1.D3)**.5D0/bt_exp/1.D4)**2
     >     *amassgas_exp**.5D0
       endif
c       write(*,44) jm, rho(jm), te_m(jm), rhosda_m(jm)
 44    format(2x,i2,2x,0p1f5.3,0p9f10.5)
c
...
c        bteff_exp(j)=bt_exp*rho(j)*arho_exp/rmin_exp(j)*drhodr(j)
c        write(*,44) j, rho(j), bteff_exp(j)
      enddo
      ...
      do j=jm-jptf, jm+jpt
        ...
        if(pfast_exp(1).gt.0) 
     >   zppf_m(j)=-(dlog(pfast_exp(j-1))-dlog(pfast_exp(j)))/drho
c        write(*,*) j, drho, zppf_m(j)
      enddo
...
c
        ...
        if (DABS(zpmpf).lt.zeps) zpmpf=zeps
c
        if(i_grad.eq.1) then
         zpmte=zpte_in
         zpmti=zpti_in
...
c include fast ion pressure if xalpha < 0
c
       if(xalpha.le.0) then
     ...
     >   (zpmni+zpmti)
     >    +zpmne+zpmte+zpmpf*pfast_exp(jm)/(nem*tem))
       endif
c        alpha_m(jm)=drhodr(jm)*
c    >    q_exp(jm)**2*rmaj_exp(jm)/arho_exp*
c    >    betae_m(jm)*((tim/tem*nim/nem)*
c    >   (zpmni+zpmti)
c    >    +zpmne+zpmte)
c      endif
c
c     write(*,53) jm,rho(jm),alpha_exp(jm),alpha_m(jm)
c     write(*,53) jm,rho(jm),pfast_exp(jm),alpha_m(jm),zpmpf
...
c banana regime ie collisionless limit formulas
     ...
     >      0.46D0*(rmin_exp(j)/rmaj_exp(j))**1.5D0
        ...
        if (jj.eq.1) alpha_neo_mid=alpha_neo
...
cx        angrotp_exp(j)=corot*angrotp_exp(j)
          ...
          if (jj.eq.1) pgeo_local_mid=drhodr(j)
...
c compute shears from outside minus inside
        ...
        j=jm
          ...
          rdrho_local_p1=rmin_exp(j+jpt)/arho_exp/rho(j+jpt)
c
c.. 3 pt spatial averaging of ve prior to computing egamma
c   compute ve at jm-1 and save as ve_low to compute ve_avg(jm)
        if (j.ge.2 .and. ave_ve.gt.0) then
          j=jm-1
          pgeo_local_low=(rho(j)-rho(j-1))*arho_exp/
     >        (rmin_exp(j)-rmin_exp(j-1)+epsilon)
          zpni_low=-(dlog(ni_m(j-1))-dlog(ni_m(j)))/(rho(j-1)-rho(j))
          zpti_low=-(dlog(ti_m(j-1))-dlog(ti_m(j)))/(rho(j-1)-rho(j))
          csda_low=9.79D5*(te_m(j)*1.D3)**.5D0/
     >        (arho_exp*100.D0)/amassgas_exp**0.5D0
          if (bt_flag .gt. 0) then
            rhosda_low=((1.02D2*(te_m(j)*1.D3)**.5D0)/bteff_exp(j)
     >        /1.D4)*amassgas_exp**.5D0/(arho_exp*100.D0)
          else
            rhosda_low=((1.02D2*(te_m(j)*1.D3)**.5D0)/bt_exp/1.D4)
     >        *amassgas_exp**.5D0/(arho_exp*100.D0)
          endif
c
          fc=1-1.46D0*(rmin_exp(j)/rmaj_exp(j))**0.5D0+
     >      0.46D0*(rmin_exp(j)/rmaj_exp(j))**1.5D0
          akappa1=0.8839D0*fc/(0.3477D0+0.4058D0*fc)
          alpha_neo_low=-akappa1+1.D0
c ve at jm-1 for itport_pt(4).eq.0 case:
          ve_low=-(ti_m(j)/te_m(j))*csda_low*arho_exp*rhosda_low*
     >       (zpni_low+alpha_neo_low*zpti_low)*vstar_sign*pgeo_local_low
     >       -rho(j)*rmin_exp(j)/arho_exp/rho(j)*
     >       arho_exp/rmajor_exp/q_exp(j)*vphi_m(j)
        endif
c
c   compute ve at jm+2 and save as ve_high to compute ve_avg(jm+1)
        if (j.le.jmaxm-2 .and. ave_ve.gt.0) then
          j=jm+2
          pgeo_local_high=(rho(j)-rho(j-1))*arho_exp/
     >        (rmin_exp(j)-rmin_exp(j-1)+epsilon)
          zpni_high=-(dlog(ni_m(j-1))-dlog(ni_m(j)))/(rho(j-1)-rho(j))
          zpti_high=-(dlog(ti_m(j-1))-dlog(ti_m(j)))/(rho(j-1)-rho(j))
          csda_high=9.79D5*(te_m(j)*1.D3)**.5D0/
     >        (arho_exp*100.D0)/amassgas_exp**0.5D0
          if (bt_flag .gt. 0) then
            rhosda_high=((1.02D2*(te_m(j)*1.D3)**.5D0)/bteff_exp(j)
     >        /1.D4)*amassgas_exp**.5D0/(arho_exp*100.D0)
          else
            rhosda_high=((1.02D2*(te_m(j)*1.D3)**.5D0)/bt_exp/1.D4)
     >        *amassgas_exp**.5D0/(arho_exp*100.D0)
          endif
c
          fc=1-1.46D0*(rmin_exp(j)/rmaj_exp(j))**0.5D0+
     >      0.46D0*(rmin_exp(j)/rmaj_exp(j))**1.5D0
          akappa1=0.8839D0*fc/(0.3477D0+0.4058D0*fc)
          alpha_neo_high=-akappa1+1.D0
c ve at jm+2 for itport_pt(4).eq.0 case:
          ve_high=-(ti_m(j)/te_m(j))*csda_high*arho_exp*rhosda_high*
     >       (zpni_high+alpha_neo_high*zpti_high)*
     >       vstar_sign*pgeo_local_high
     >       -rho(j)*rmin_exp(j)/arho_exp/rho(j)*
     >       arho_exp/rmajor_exp/q_exp(j)*vphi_m(j)
        endif
c
        j=jm
c
        if (j.lt.jmaxm-1) then
          ve_avg(j)=(1.D0-ave_ve)*ve(j)+
     >              ave_ve*(ve_low+ve(j+1))/2.D0
        endif
c
        if (j.lt.jmaxm-1) then
          ve_avg(j+1)=(1.D0-ave_ve)*ve(j+1)+
     >              ave_ve*(ve(j)+ve_high)/2.D0
        endif
        if (j.eq.1) then
          ve_avg(j)=ve(j)
          ve_avg(j+1)=ve(j+1)
...
c
c.. end smoothing of ve
c
        if (ave_ve.gt.0) then
        egamma_avg(jm)=relx*egamma_avg(jm)+(1.D0-relx)*(
     >  egeo_local*drhodrrrho(j)*
     >  (rho(j-jptf)+rho(j+jpt))/(q_exp(j-jptf)+q_exp(j+jpt))*
     >  (ve_avg(j+jpt)*q_exp(j+jpt)/rho(j+jpt)/rdrho_local_p1-
     >  ve_avg(j-jptf)*q_exp(j-jptf)/rho(j-jptf)/rdrho_local)/
     >  (rho(j+jpt)-rho(j-jptf)+epsilon)/arho_exp/csda_m(j)
     >  )
        egamma_m(jm)=egamma_avg(jm)
c
        else
...
c
c       write(*,23) j, rho(j), ve_avg(j), ve_avg(j+1),
c    >              egamma_m(j), egamma_avg(j)
c23     format(i2,2x,0p1f6.4,1x,0p5e13.5,2x,'ve, egamma')
c
...
c     > (alog(q_exp(jm-1))-alog(q_exp(jm)))/(rho(jm-1)-rho(jm))
       ...
       if(alpha_gf.gt.4.0) alpha_gf=4.D0
...
c       write(*,200) jm, rho(jm), rmin_gf, rmaj_gf, elong_gf, drhodr(jm),
c    >     dil_gf
c       write(*,200) jm, rho(jm), rlti_gf, rlte_gf, rlne_gf, rlni_gf,
c    >    rlnimp_gf
c       write(*,200) jm, rho(jm), zpmti, zpmte, zpmne, zpmni, zpmnimp
c       write(*,200) jm, rho(jm), q_gf, shat_gf, taui_gf, xnu_gf
c       write(*,200) jm, rho(jm), alpha_gf, alpha_p_gf, alpha_e_gf
c       write(*,*) jm, csda_m(jm)
...
c       diff_k_j(ik,jm) = diff_k_gf(ik)
c       chie_itg_j(ik,jm) = chie_k_gf(ik)                   ! ITG/TEM
c       chie_etg_j(ik,jm) = chie_e_k_gf(ik)                 ! ETG
c       chie_k_j(ik,jm) = chie_k_gf(ik) +  chie_e_k_gf(ik)  ! ITG/TEM & ETG
c       chii_k_j(ik,jm) = chii_k_gf(ik)                     ! ITG/TEM
c       write(*,'(i3,2x,i3,1p8e11.3)') jm, ik, xkyf_k_gf(ik),
c    >       gamma_k_gf(1,ik), freq_k_gf(1,ik),
c    >       chie_k_gf(ik), chii_k_gf(ik)
c    >       gamma_k_j(ik,jm), freq_k_j(ik,jm),
c    >       chie_itg_j(ik,jm), chie_etg_j(ik,jm), 
...
c      dnfreq_m(jm)=0.
       ...
       gfac=geofac(jm)
c
c gfac used in ptor_dv:
c
       if (igfac .gt. 0) then
         gfac=gfac*0.25D0*(gradrhosq_exp(jm+1)+gradrhosq_exp(jm))*
     >        (sfactor(jm+1)+sfactor(jm))/max(vprime(jm,2),1.D-6)
       endif
c
c exch_m in MW/m**3
c   kev/sec per MW
