      subroutine ip_chi2(itest_ntcc,itest_chi,
     .     RLT,RLN,RLNe,q,kappa,
     .     shat,zth,nbeam,tau,eps,gnu,
     .     g_perp,rmajor,rho_i,v_ti,
     .     RLTcrit,RLTcritz,chi_0,g,
     .     gamma,chi_i,chi_e)
c
...
c
      implicit none
      integer itest_chi, itest_ntcc
c
      real*8 RLT,RLN,RLNe,shat,zth,tau,rminor,rmajor,radius,chi_i,q,
     .     nbeam,taub,rho_i,v_ti,eps,chi_e,nu,chi_0,g,g_perp,gnu
      real*8 gamma,rot
      real*8 RLTcrit,RLTcritz,chi0,a_0,b_0,RLTcrit2,RLN_th
      real*8 f_0,f_z,chiz,g_facz,z_0,c_0
      real*8 c1,c2,trln,trlne,tshat,kappa
      real*8 chie1,chie2
      real*8 g_fac1,g_fac2,zx
      data a_0/0.D0/
      data b_0/0.D0/
      data c_0/1.D0/
c      data a_0/-1.D0/
...
c
      ...
      nu=gnu*0.84D0
c 
      tRLN=min(abs(RLN),6.D0)*sign(1.D0,RLN)
      tRLNe=min(abs(RLNe),6.D0)
c
c Formula is not applicable for shat<0.5; doesn_t matter most places.
c
      tshat=max(shat,0.5D0)
c
c     critical ion temperature gradient:
c
      RLTcrit=2.46D0*(1.D0+2.78D0/q**2)**0.26D0*
     .     (zth/2.D0)**0.7D0*taub**0.52D0
     .     *( (0.671D0+0.570D0*tshat-0.189D0*tRLN)**2
     .     +0.335D0*tRLN+0.392D0-0.779D0*tshat+0.210D0*tshat**2)
     .     *( 1.D0-0.942D0*(2.95D0*eps**1.257D0/nu**0.235D0-0.2126D0)
     .     *zth**0.516D0/abs(tshat)**0.671D0)
c
      RLTcritz=0.75D0*(1.D0+taub)*(1.D0+tshat)*
     .     max(1.D0,3.D0-2.D0*tRLNe/3.D0)*
     .     (1.D0+6.D0*max(0.D0,2.9D0-zth))
c
      c1=1.D0
      if(zth.gt.3.) c1=(3.D0/zth)**1.8D0
c
      f_0=11.8D0*c1*q**1.13D0/(1.D0+tshat**0.84D0)/taub**1.07D0
     .     *(1.D0+6.72D0*eps/nu**0.26D0/q**0.96D0)
     .     /(1.D0+((kappa-1.D0)*q/3.6D0)**2)
c  
      f_z=7.88D0/(1.D0+tshat)*max(0.25D0,zth-3.D0)/taub**0.8D0
     .     /(1.D0+((kappa-1.D0)*q/3.6D0)**2)
c        
...
c
      if(RLT-RLTcrit.gt.0.) then
         g_fac1=min((RLT-RLTcrit)**0.5D0,(RLT-RLTcrit))
      else
         g_fac1=0.D0
      endif
c
      if(RLT-RLTcritz.gt.0.) then
         g_facz=min((RLT-RLTcritz)**0.5D0,(RLT-RLTcritz))
      else
         g_facz=0.D0
      endif
c
      if(itest_ntcc.gt.0) then
         zx=RLT-RLTcrit
         g_fac1=zx
         if(zx.lt.0.D0) g_fac1=0.D0
         if(zx.gt.1.D0) g_fac1=dsqrt(zx)
c
         zx=RLT-RLTcritz
         g_facz=zx
         if(zx.lt.0.D0) g_facz=0.D0
         if(zx.gt.1.D0) g_facz=dsqrt(zx)
      endif
c
      chi_i=max(chi0*g_fac1,chiz*g_facz)
c
      g=RLT/RLTcrit
      chi_0=chi0*dsqrt(abs(RLTcrit))
c
      chie1=chi0*g_fac1*1.44D0*tau**0.4D0*(q/tshat)**0.3D0*nu**0.14D0
     .     *max(0.16667D0,eps)
      chie1=0.5D0*chie1*(1.D0+tRLNe/3.D0)
c
      chie2=0.5D0*max(2.D0,(1.D0+RLNe/3.D0))
      chie2=chie2*0.526D0*tau*nu**0.22D0
      chie2=chie2*chiz*g_facz
c
c Correction for n_i/n_e and ratio of heat fluxes rather than chi_s:
c
      chi_e=max(chie1,chie2)*(7.D0-zth)/6.D0
c
...
c
      gamma=0.25D0/(1.D0+0.5D0*tshat**2)/tau
     .     *(1.D0+3.D0*max(0.D0,eps-0.16667D0))
     .     /(1.D0+max(0.D0,q-3.D0)/15.D0)
     .     *(RLT-RLTcrit)*v_ti/rmajor
c
      rot=min(1.D0,max(0.D0,(1.D0-abs(g_perp)/gamma)))
c
      chi_i=max(0.D0,c_0*chi_i*rot)
      chi_e=max(0.D0,c_0*chi_e*rot)
c
c test
