c@callglf2d.f
c 09Dec08   HSJ     added chie_etg_m,chie_etg_tem,gamma_net_m
...
************************************************************************
     ...
     > chie_etg_tem,   ! etg part of chietem
     ...
     >  diff_m,
     >  chie_m,
     >  chie_etg_m,    ! ETG part of chie_m (ETG is also included in chie_m)
     >  chii_m,
     >  etaphi_m,
     >  etapar_m,
     >  etaper_m,
     ...
     >  gamma_net_i,   !0:jmaxm max net growth rate,taken over all low k, ion modes all equations(8)) HSJ
     >  gamma_net_e    !0:jmaxm max net growth rate,(taken over all high k electron modes , all equations(6)) HSJ  
     > )
c************************************************************************
...
c      only common used for communication with glf2d....designation by xxxxx_gf
      ...
      include 'glf.m'  ! added gamma_net_max_i,e HSJ 9/12/08
...
c external arrays
     ...
     >  diffnem, chietem, chiitim, etaphim,chie_etg_tem,
     ...
     >  etaphi_m(0:jmaxm),chie_etg_m(0:jmaxm),
     ...
     >  gamma_net_i(0:jmaxm),gamma_net_e(0:jmaxm)
...
c--- indecies 1 to 4 below correspond to
c--- equation numbers in descending order of growth rate. Where ,for each equation
c--- the maximum growth rate  over the ky spectrum is used. See glf2d for dependent
c--- variable that each equation corresponds to. (nmode = total
c--- number of modes, including etg but etg is turned  off by xparam(10))   HSJ
c
c      ky_j(jm)=xkyf_gf
...
c      enddo
       ...
       gamma_net_i(jm) = gamma_net_max_i
       gamma_net_e(jm) = gamma_net_max_e
       anrate2_m(jm)=gamma_gf(2)
c      dnrate_m(jm)=0.
c      dtnrate_m(jm)=0.
       anfreq_m(jm)=freq_gf(1)
       anfreq2_m(j)=freq_gf(2)
       anfreq2_m(jm)=freq_gf(2)   ! HSj corrected 12/8/08
c      dnfreq_m(jm)=0.
...
c ITER definition of "chi" assumes will be proceeded with gradrhosq factor
      ...
      chie_etg_tem=cmodel*gfac*chie_e_gf*cgyrobohm_m(jm)
...
c         write(6,*) jm,rho(jm),egamma_m(jm), 'egamma_m-e'




        enddo  grid_loop !jm index
...
chsj        call echo('chie_etg_m output',chie_etg_m,jmaxm)
...
chsj        call echo('gamma_net_i output',gamma_net_i,jmaxm)
chsj        call echo('gamma_net_e output',gamma_net_e,jmaxm)
...
c
c        write(6,*) jm-1,rho(jm-1),egamma_m(jm-1), 'egamma_m-e1'
       return
       end
c
