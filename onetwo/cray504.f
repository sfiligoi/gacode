...
!Declaration of input variables
     ...
     #               p_grbm2,                 p_grphi,
     #               p_gr2phi,                p_ngrth
      REAL*8         amu_i(mx_mi),            grt_i(mx_mi),
     #               temp_i(mx_mi)
      REAL*8         den_iz(mx_mi,mx_mz),     fex_iz(3,mx_mi,mx_mz),
     #               grp_iz(mx_mi,mx_mz)
!Declaration of output variables
      INTEGER        iflag,                   m_s
      INTEGER        jm_s(mx_ms),             jz_s(mx_ms)
      REAL*8         p_bsjb,                  p_etap,
     #               p_exjb
      REAL*8         calm_i(3,3,mx_mi)
      REAL*8         caln_ii(3,3,mx_mi,mx_mi),capm_ii(3,3,mx_mi,mx_mi),
     #               capn_ii(3,3,mx_mi,mx_mi)
      REAL*8         bsjbp_s(mx_ms),          bsjbt_s(mx_ms),
     ...
     #               ymu_s(3,3,mx_ms)
      REAL*8         chip_ss(mx_ms,mx_ms),    chit_ss(mx_ms,mx_ms),
...
!Declaration of local variables
     ...
     #               k,                       l
      REAL*8         dent
      REAL*8         z_coulomb,               z_electronmass,
     #               z_j7kv,                  z_mu0,
     #               z_pi,                    z_protonmass
      REAL*8         denz2(mx_mi),            vt_i(mx_mi)
      REAL*8         pgrp_iz(mx_mi,mx_mz)
      REAL*8         amnt_ii(mx_mi,mx_mi)
      REAL*8         tau_ss(mx_ms,mx_ms)
!Initialization
!  Error flag
      iflag=0
c     write(6,*) 'm_i-ncl = ',m_i
c     write(6,*) 'm_z-ncl = ',m_z
c     write(6,*) 'mx_mi-ncl = ',mx_mi
c     write(6,*) 'c_den-ncl = ',c_den
c     write(6,*) 'c_potb-ncl = ',c_potb
c     write(6,*) 'c_potl-ncl = ',c_potl
c     write(6,*) 'p_b2-ncl = ',p_b2
c     write(6,*) 'p_bm2-ncl = ',p_bm2
c     write(6,*) 'p_fhat-ncl = ',p_fhat
c     do im=1,m_i
c       write(6,*) im, ' amu_i-ncl = ',amu_i(im)
c       write(6,*) im, ' temp_i-ncl = ',temp_i(im)
c       write(6,*) im, ' grt_i-ncl = ',grt_i(im)
c       write(6,*) im, ' den_iz-ncl = ',(den_iz(im,k),k=1,m_z)
c       write(6,*) im, ' grp_iz-ncl = ',(grp_iz(im,k),k=1,m_z)
c     enddo
!  Consistency checks
...
!Declaration of input variables
     ...
     #               p_eb,                    p_fhat,
     #               p_grbm2
      REAL*8         temp_i(mx_mi),           grt_i(mx_mi),
     #               calm_i(3,3,mx_mi)
      REAL*8         caln_ii(3,3,mx_mi,mx_mi)
      REAL*8         den_iz(mx_mi,mx_mz),     grp_iz(mx_mi,mx_mz),
     #               fex_iz(3,mx_mi,mx_mz)
      REAL*8         xi_s(mx_ms),             ymu_s(3,3,mx_ms)
!Declaration of output variables
      INTEGER        iflag
      REAL*8         p_bsjb,                  p_etap,
     #               p_exjb
...
!Declaration of local variables
     ...
     #               z_coulomb,               z_j7kv
      REAL*8         aa(3,3),                 xl(3)
      REAL*8         crhat(3,6,mx_mi),        rhat(3,6,mx_ms)
      REAL*8         srcth(3,mx_ms),          srcthp(mx_ms),
     #               srctht(mx_ms)
      REAL*8         ab(3*mx_mi,3*mx_mi),     xab(3*mx_mi,3)
      REAL*8         crhatp(3,mx_ms,mx_mi),   crhatt(3,mx_ms,mx_mi)
      REAL*8         rhatp(3,mx_ms,mx_ms),    rhatt(3,mx_ms,mx_ms),
     #               uaip(3,mx_ms,mx_ms),     uait(3,mx_ms,mx_ms)
      REAL*8         xabp(3*mx_mi,mx_ms),     xabt(3*mx_mi,mx_ms)
!Initialization
!  Error flag
      iflag=0
!  Physical and conversion constants
...
!Declaration of input variables
     ...
     #               k_potato,                m_s
      INTEGER        jm_s(mx_ms),             jz_s(mx_ms)
      REAL*8         c_potb,                  c_potl,
     #               p_fm(3),                 p_ft,
     #               p_ngrth,                 x
      REAL*8         amu_i(mx_mi),            temp_i(mx_mi),
     #               vt_i(mx_mi)
      REAL*8         sqz_s(mx_ms)
!Declaration of output variables
      REAL*8         ykb_s(mx_ms),            ykp_s(mx_ms),
...
!Declaration of local variables
     ...
     #               z_coulomb,               z_pi,
     #               z_protonmass
      REAL*8         ynud_s(mx_ms),           ynut_s(mx_ms),
     #               ynutis(3,mx_ms)
!Initialization
!  Physical and conversion constants
      z_coulomb=1.6022e-19
...
!Declaration of input variables
      ...
      REAL*8         amu_i(mx_mi),            temp_i(mx_mi)
!Declaration of output variables
      REAL*8         capm_ii(3,3,mx_mi,mx_mi), capn_ii(3,3,mx_mi,mx_mi)
!Declaration of local variables
      INTEGER        im,                      jm
      REAL*8         xab,                     xab2,
     #               xmab,                    xtab,
...
!Declaration of input variables
     ...
     #               p_fm(3)
      REAL*8         amu_i(mx_mi),            temp_i(mx_mi),
     #               vt_i(mx_mi)
      REAL*8         den_iz(mx_mi,mx_mz)
      REAL*8         sqz_s(mx_ms)
!Declaration of output variables
      REAL*8         ymu_s(3,3,mx_ms)
      REAL*8         tau_ss(mx_ms,mx_ms)
!Declaration of local variables
      INTEGER        mpnts
      PARAMETER     (mpnts=13)
...
!Declaration of input variables
      ...
      INTEGER        jm_s(mx_ms)
      REAL*8         p_ngrth,                  x
      REAL*8         temp_i(mx_mi),            vt_i(mx_mi)
      REAL*8         tau_ss(mx_ms,mx_ms)
!Declaration of output variables
      REAL*8         ynud_s(mx_ms),            ynut_s(mx_ms),
     #               ynutis(3,m_s)
...
!Declaration of local variables
     ...
     #               z_epsilon0,              z_pi,
     #               z_protonmass
      REAL*8         xlnab(mx_mi,mx_mi),      xn(mx_mi),
     #               xnz(mx_mi),              xz(mx_mi),
     #               xnz2(mx_mi)
!Initialization
!  Physical and conversion constants
...
!Coulomb logarithm
      DO i=1,m_s
        ...
        IF(iz.lt.0) THEN
!         Electrons
          clnab=37.8-LOG(SQRT(den_iz(im,iza))/temp_i(im)) ! was DLOG
        ENDIF
...
!  Ions
            xlnab(im,jm)=40.3
     #               -LOG(xz(im)*xz(jm)*(amu_i(im)+amu_i(jm)) ! was DLOG
