      subroutine iter_dbase

c
c ----------------------------------------------------------------------
c --- collects and writes out (or reads in) data for ITER database
c --- this file (but not the netcdf version) has  new required 
c --- additions for the NTCC transport module GCNMP (8-8-2005)  HSJ
c
c --- INPUT
c
...
c     triangnpsi_l(j)  triangularity
...
c
      ...
      USE iterdbmd
      USE etc
      USE flags,              ONLY  : inenez
      USE neo2d,              ONLY  : xips,xhm2,xi11,xi33,eps
      USE bd_condtn,          ONLY  : bctime,totcur,rtein,tein,
     .                                knotste,rtiin,tiin,knotsti,
     .                                zeffin,rzeffin,knotszef,
     .                                enein,renein,knotsene,dnedt,
     .                                dnidt
...
c
     ...
     .           fspline(kpsi), bpar(4), work(kj)
      REAL *8, allocatable,dimension(:) ::  te_bc,ti_bc,angrot_bc,
     .         zeff_bc,ene_bc
      REAL *8, allocatable,dimension(:,:) :: en_bc 
      logical    monotonic
...
c
    ...
    1     format ('** shot number = ', i5, '  time = ', 1pe14.6)
c
...
c
           if (irwflag .eq. 0) then
               write  (niterdb, 3086)
 3086          format ('*  sbeam : beam thermal ion source,',
     .                 ' #/(meter**3*second)')
...
!
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 4120)
 4120   FORMAT ('*  toroidal electric field profile, V/m')
        WRITE  (niterdb, 10) (etor(j)*100., j=1,nj)
      ELSE
        READ   (niterdb,  7) starflag
        READ   (niterdb, 10) (etor(j), j=1,nj)
        etor(:) = etor(:)*1.e-2
      END IF
...
c
 ...
 1210   format ('*  r points for plasma boundary, meters')
        write  (niterdb, 10) (rplasbdry(j)*0.01, j=1,nplasbdry)
        if (iterdsc .ne. 0)
     .  write  (niterdb, 1215)
 1215   format ('*  z points for plasma boundary, meters')
        write  (niterdb, 10) (zplasbdry(j)*0.01, j=1,nplasbdry)
      else
        read   (niterdb,  7)  starflag
...
! ----------------------------------------------------------------------
! new required additions for GCNMP (8-8-2005)  HSJ
! mostly these are quantites required to compute bounday conditions
! ----------------------------------------------------------------------
       IF(create_GCNMP_input)then
         ICALL = ICALL -1       ! dont append in these cases
       ...
       tGCNMf = timmax
       IF (irwflag .EQ. 0) THEN
          IF (iterdsc .NE. 0) WRITE  (niterdb, 3640)
 3640    FORMAT ('* end of simulation time,sec')
       ...
       IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3642)
 3642   FORMAT ('* boundary condition time ,sec')
       ...
       END IF
        ...
        read(label,FMT='(a)')asc_time
        asc_time = ADJUSTL(asc_time)

      totcur_bc = totcur(nbctim)
      if(totcur_bc .le. 0.0_DP) totcur_bc = totcur(1)
      IF (irwflag .EQ. 0) THEN
        label = '* boundary condition total current,amps,at time '   
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
      ...
      vloop_bc = 0.0   !option not implmented change iterdb manually for this
      IF (irwflag .EQ. 0) THEN
        label = '* boundary condition loop voltage,Volts,at time '
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
      ...
      IF (irwflag .EQ. 0) THEN
        fix_edge_te_bc  = fix_edge_te(nbctim)
        fix_edge_ti_bc  = fix_edge_ti(nbctim)
        fix_edge_rot_bc = fix_edge_rot(nbctim)
        fix_edge_ni_bc  = fix_edge_ni(nbctim)
...
c     get te at time time_bc for use as BC in GCNMP
!      call set_boundary_condition (time_bc) !determines the BC values equired
      IF (irwflag .EQ. 0) THEN
        call intrp (-1,-1,rtein(1,nbctim),tein(1,nbctim),
     .               knotste(nbctim),roa,te_bc,nj)
        label = '* boundary condition TE,kev,at time '   
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
...
c     get ti at time time_bc for use as BC in GCNMP
      IF (irwflag .EQ. 0) THEN
        call intrp (-1,-1,rtiin(1,nbctim),tiin(1,nbctim),
     .                      knotsti(nbctim),roa,ti_bc,nj)
        label = '* boundary condition TI,kev,at time '   
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
...
c     get ene  at time time_bc for use as BC in GCNMP
      IF (irwflag .EQ. 0) THEN
        If(inenez .eq. 1)then
           call intrp (-1,-1,renein(1,nbctim),enein(1,nbctim),
     .                     knotsene(nbctim),roa,ene_bc,nj)
           ene_bc  = ene_bc*1.0e+6
        else
          !electron density not available at time bctime(nbctim)
          !because we are running in ion mode. Indicate this by
          !putting out  zero density:
          ene_bc(1:nj) = 0.0
        endif

        label ='* bc profile: ene, 1/m**3 at time  '
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
...
c     get zeff   at time time_bc for use as BC in GCNMP
      IF (irwflag .EQ. 0) THEN
        if(inenez .eq. 1 )then
           call intrp (-1,-1,rzeffin(1,nbctim),zeffin(1,nbctim),
     .                     knotszef(nbctim),roa,zeff_bc,nj)
        else
          zeff_bc(1:nj) = 0.0
        endif

        label = '* bc profile: zeff, at time  '
     .          //asc_time(1:LEN_TRIM(asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                   label(1:LEN_TRIM(label))
...
c     get toroidal rotation    at time time_bc for use as BC in GCNMP
c     get rotation  at time time_bc for use as BC in GCNMP
      IF (irwflag .EQ. 0) THEN
        call intrp (-1,-1,rangrot(1,nbctim),angrotin(1,nbctim),
     .             knotsang(nbctim),roa,angrot_bc,nj)
...
! --- get all densities   at time time_bc for use as BC in GCNMP
! --- At present Onetwo does not have time dependent density input
! --- So for GCNMP we jsut write out the current density values
       jp = 0
       ji = 0
       DO jj=1,nion
           IF (jj .LE. nprim)  jp = jp + 1
     ...
     .          WRITE  (niterdb, 3055) namep(jp)
               IF (iterdsc .NE. 0 .AND. jj .GT. nprim) 
     .               WRITE  (niterdb, 3056) namei(ji)
               WRITE  (niterdb, 10) (en(j,jj)*1.e6, j=1,nj)
           ELSE
               READ   (niterdb,  7) starflag
...
!
      ...
      IF (irwflag .EQ. 0) THEN
        ...
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                              label(1:LEN_TRIM(label))
         WRITE  (niterdb, 10) (wbeam(j)*1.e6, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
      ...
      IF (irwflag .EQ. 0) THEN
        ...
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                                label(1:LEN_TRIM(label))
         WRITE  (niterdb, 10) (walp(j)*1.e6, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
       READ   (niterdb, 10) (walp(j), j=1,nj)
      END IF
      ...
      IF (irwflag .EQ. 0) THEN
        ...
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .                            label(1:LEN_TRIM(label))
         WRITE  (niterdb, 10) (enalp(j)*1.e6, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
     ...
     .             //st_asc_time(1:LEN_TRIM(st_asc_time))
        IF (iterdsc .NE. 0) WRITE(niterdb,FMT='(A)')
     .      label(1:LEN_TRIM(label))
         WRITE  (niterdb, 10) (dnedt(j)*1.e6, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
       READ   (niterdb, 10) (dnedt(j), j=1,nj)
      END IF
...
C     get horizontal inverse aspect ratio = (rmax-rmin)/(rmax+rmin)
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3126)
 3126   FORMAT 
     .  ('* horizontal inverse aspect ratio = (rmax-rmin)/(rmax+rmin)')
      ...
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3660)
 3660   FORMAT ('*rcap = < R>,m')
        WRITE  (niterdb, 10) (rcap(j)*0.01, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
       READ   (niterdb, 10) (rcap(j), j=1,nj)
      END IF



      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3670)
 3670   FORMAT ('*r2capi = <R**2>       M**2')
        WRITE  (niterdb, 10) (r2capi(j)*1.e-4, j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
       READ   (niterdb, 10) (r2capi(j), j=1,nj)
      END IF


      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3672)
 3672   FORMAT ('*r2cap = <R0**2/R**2> ')
        WRITE  (niterdb, 10) (r2cap(j), j=1,nj) 
      ELSE
       READ   (niterdb,  7) starflag
       READ   (niterdb, 10) (r2cap(j), j=1,nj)
      END IF

!     some quantities required for trapped particle fraction, and
!     neocl;assical transport
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3680)
 3680   FORMAT ('*xhm2 = < (B total/ B axis)**2 > ',
     .          '(=1 for circular plasmas)')
      ...
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3690)
 3690   FORMAT ('*xi11 ( = 1.95 sqrt(eps)for circular plasmas)' )
      ...
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3700)
 3700   FORMAT ('*xi33 ( = 1.95 sqrt(eps)for circular plasmas)')
      ...
      IF (irwflag .EQ. 0) THEN
        IF (iterdsc .NE. 0) WRITE  (niterdb, 3710)
 3710   FORMAT ('*xips = <(Baxis/B)**2)> - 1./(<(B/Baxis)**2> )'
     .      '( = 2 eps**2 for circular plasmas)')
      ...
      DEALLOCATE(ene_bc)
      DEALLOCATE(zeff_bc)
      DEALLOCATE(angrot_bc)
      DEALLOCATE(en_bc)


      ENDIF    !    create_GCNM_input
