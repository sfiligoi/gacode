...
contains
  ...
  subroutine ThreeD_do
    ...
    do i=1,matsize
       do j=1, matsize
          read(1,*) tpmat_trap(i,j), tpmat_stream_dt(i,j), &
               tpmat_stream_dp(i,j), tpvec_fsa(i,j)
       enddo
    enddo
    ...
    if (silent_flag == 0 .and. i_proc == 0) then
       ...
       write(io_neo,*) n_xi
       write(io_neo,*) n_tptheta
       write(io_neo,*) n_tpvarphi
...
200 call ENERGY_basis_ints_alloc(0)
  ...
  subroutine threed_matrix_solve(ir,status)
    ...
    n_row = n_species*(n_energy+1)*(n_xi+1)*tp_matsize
    ! Estimate number of elements
    i = 0
    ! constraint
    if(collision_model == 1 .or. collision_model == 2) then
       i = i + n_species*tp_matsize*(n_energy+1)
    else
       i = i + n_species*tp_matsize*2
    endif
    ! collisions
    i = i + n_species*(n_xi+1)*tp_matsize*(n_energy+1)**2 &
         * (1 + n_species)
    ! streaming (theta and varphi derivs)
    i = i + 2*n_species*(tp_matsize**2)*(n_energy+1)**2*((n_xi-1)*4*2 + 2*4)
    ! trapping
    i = i + n_species*(tp_matsize**2)*(n_energy+1)**2*((n_xi-1)*2 + 2)
    n_max = i
    ...
    allocate(g(n_row))
    allocate(mindx(n_species,0:n_energy,0:n_xi,tp_matsize))
...
100 continue
  ...
  subroutine threed_transport(ir)
    ...
    allocate(upar(n_species,n_tpmatsize))
    ...
    jpar = 0.0
    do is=1, n_species
       jpar = jpar + Z(is) * uparB(is) * dens(is)
    enddo
