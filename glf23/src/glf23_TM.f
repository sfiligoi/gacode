...
cgms#include "f77_dcomplx.h"
 
c
cglf2d.f 02-feb-01 Kinsey
c
c GACDODE version Sep. 25, 2014 G.M. Staebler
c
c---:----1----:----2----:----3----:----4----:----5----:----6----:----7-c
c
...
c
c      real*8 anorm_glob,diff_glob,diff_im_glob,chii_glob
c     &       ,chie_glob, exch_glob,eta_par_glob,eta_per_glob
c     &       ,eta_phi_glob,chie_e_glob
c      real*8 chie_e_k_gf_glob(20), chie_k_gf_glob(20),
c     &       chie_tot_gf(20), xkyf_k_gf_glob(20)
c      real*8 m(2), m_global(2)
c
c     external f02ake
c
c---:----1----:----2----:----3----:----4----:----5----:----6----:----7-c
...
c
cgms      if(ipert_gf.eq.1.and.ngrow_k_gf(0).eq.0)go to 888   
c
...
c
     ...
     >  /(1.D0+xparam(18)*sqrt(betae/.006D0))
       if(iflagin(5).eq.6) rms_theta=rms_theta_gf/
     >      (xparam(13)*((q_gf/2.D0)-1.D0)+1.D0)
     >  /sqrt(1.D0+xparam(16)*((shat_gf-
     >  xparam(19)*alpha)**2-0.5D0)+xparam(17)*
     >  (shat_gf-xparam(19)*alpha-0.5D0)**2)/
...
c ilh=1 for low k ion modes, ilh=2 high k electron modes
c
      do iky0=1,ikymaxtot
c
cgms      iky=iky0
...
c
cgms      if(ipert_gf.eq.1.and.ngrow_k_gf(iky0).eq.0)go to 777 
c skip this k if the previous k was stable and 4 k's have been done      
cgms      if(iky.lt.ikymax_gf-4.and.ngrow_k_gf(iky0-1).eq.0)go to 777
c 
c primary ions
c 
      b0=taui*k_per**2
...
c 
c... jek alpha-stab. fix for NCS (renorm only)
c
c     if(ilh.eq.1 .and. iky0.eq.2) write(*,*) 'ave_g = ',ave_g,ky
c
      w_s=ky
...
c      if(iky0.eq.1 .and. ilh.eq.1) write(*,*) reps
c 
...
c
        ifail = 0
c
cgms        if (eigen_gf .eq. 2 ) then
cgms
cgms        jobvl = 'N'
cgms        jobvr = 'V'
cgms        do j1=1,neq
cgms         do j2=1,ieq
cgms           mata(j1,j2) = dcmplx(ar(j1,j2),ai(j1,j2))
cgms         enddo
cgms        enddo
cgms
cgms        call zgeev(jobvl,jobvr,ieq,mata,neq,w,cvl,neq,cvr,
cgms     &             neq,work,lwork,rwork,ifail)
cgms        do j1=1,neq
cgms         rr(j1) = real(w(j1))
cgms         ri(j1) = dimag(w(j1))
cgms         do j2=1,ieq
cgms           vr(j1,j2) = real(cvr(j1,j2))
cgms           vi(j1,j2) = dimag(cvr(j1,j2))
cgms         enddo
cgms        enddo
cgms
cgms        elseif (eigen_gf .eq. 1 ) then
cgms
cgms        do j2=1,neq
cgms           do j1=1,neq
cgms              bi(j1,j2)=0.0D0
cgms              if(j1.eq.j2) then
cgms                 br(j1,j2)=1.0D0
cgms              else
cgms                 br(j1,j2)=0.0D0
cgms              endif
...
cgms        enddo
cgms
cgms        call r8tomsqz(neq,ieq,ar,ai,br,bi, rr,ri,beta_tom, vr,vi, ifail)
cgms
cgms        do j1=1,ieq
cgms           ztemp1 = beta_tom(j1)
cgms           if ( abs(beta_tom(j1)) .lt. epsilon ) ztemp1 = epsilon
cgms           rr(j1)=rr(j1) / ztemp1
cgms           ri(j1)=ri(j1) / ztemp1
cgms        enddo
cgms
cgms        else
cgms
...
cgms       if(ipert_gf.eq.0)ngrow_k_gf(iky0)=1
      endif
c 
...
cgms      if(ipert_gf.eq.0)then
cgms        do j=1,nmode
cgms         if(ngrow_k_gf(j).ne.0)ngrow_k_gf(0)=1
cgms        enddo
cgms        if(ngrow_k_gf(0).eq.0)go to 888
cgms      endif
c
c
c***********************************************************************
...
c
       gamma_r_gf=gamma_r
c
c       write(*,*) gamma_gf(1), gamma_gf(2), xky_gf(1), xky_gf(2)
c
c print to file log
...
c
c compute contributions from individual k's
c
      do iky=1,ikymax_gf
       del_k=xkyf_k_gf(iky)
