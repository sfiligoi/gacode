subroutine realfluct6(nr,nth,nn,nx,ny,nz,c,f)

  implicit none
  
  integer, intent(in) :: nr,nn,nth,nx,ny,nz
  double complex, intent(in) :: c(0:nr-1,0:nth-1,0:nn-1)
  double precision, intent(inout) :: f(0:nx-1,0:ny-1,0:5)
...
!$omp parallel do private(j,i,fsum,n,p)
  do j=0,ny-1
     do i=0,nx-1
        ...
        do n=0,nn-1
           do p=0,nr-1
              fsum = fsum+real(c(p,nth/2,n)*epx(p,i)*eny(n,j))
           enddo
        enddo
        f(i,j,0) = fsum
     enddo
...
!$omp parallel do private(j,i,fsum,n,p)
  do j=0,ny-1
     do i=0,nx-1
        ...
        do n=0,nn-1
           do p=0,nr-1
              fsum = fsum+real(c(p,0,n)*epx(p,i)*eny(n,j))
           enddo
        enddo
        f(i,j,1) = fsum
     enddo
