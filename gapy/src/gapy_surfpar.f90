...
contains
  ...
  subroutine surfpar_do(n_psi,n_arc,r_in,z_in)
    ...
    double precision, dimension(:), allocatable :: r,z,dl,ur,uz,l,vr,vz,x
    ...
    ur(1:n_arc-1) = cshift(ur(1:n_arc-1),-s)
    uz(1:n_arc-1) = cshift(uz(1:n_arc-1),-s)
    dl(1:n_arc-1) = cshift(dl(1:n_arc-1),-s)
    ur(n_arc) = ur(1)
    uz(n_arc) = uz(1)
    dl(n_arc) = dl(1)
    ...
    do i=1,n_arc-1
       ...
       if (uz(i+1) > uz(i) .and. l(i)/l(n_arc) > 0.5) then
          vz(i+1) = 2*pi+uz(i+1)
       endif
    enddo
