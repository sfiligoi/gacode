pro spectrum_n_see, group=group

  common GLOBAL

  ;;-----------------------------------------------
  ;; Private (local) data  
  ;;
  common PRIVATE_SPECTRUM_N,$
    zoom,$
    active_spectrum_n,$
    widget,$
    pname0

  active_spectrum_n = 0
  zoom = 1.0
  ;;-----------------------------------------------

  ;;----------------------------------------------------------
  ;; Return conditions:
  ;;
  if exists_kxkyspec eq 0 then return    
  if xregistered('spectrum_n_see') then return
  ;;----------------------------------------------------------

  base = widget_base(title=simdir,$
                     /column)

  ;;----------------------------------------------------------
  ;; BUTTONS
  ;;----------------------------------------------------------

  row1 = widget_base(base,$
                     /row,$
                     /frame)


  w1 = widget_button(row1,$
                     value='Individual-n',$
                     uvalue=1)

  w2 = widget_button(row1,$
                     value='Comparison',$
                     uvalue=2)

  w3 = widget_button(row1,$
                     value='+n',$
                     uvalue=3)

  w4 = widget_button(row1,$
                     value='-n',$
                     uvalue=4)

  w5 = widget_button(row1, $
                     value='ZOOM in', $
                     uvalue=5)

  w6 = widget_button(row1, $
                     value='ZOOM out', $
                     uvalue=6)

  w7 = widget_button(row1, $
                     value='PS dump', $
                     uvalue=7)

  w8 = widget_button(row1, $
                     value='data', $
                     uvalue=8)

  w9 = widget_button(row1,         $
                     value='Done', $
                     uvalue=9)

  ;;----------------------------------------------------------
  ;; DRAW WIDGET and CONTROL
  ;;----------------------------------------------------------

  draw = widget_draw(base,     $
                     xsize=sx, $
                     ysize=sy)

  widget_control, base, $
    set_uvalue=state,$
    /no_copy, $
    /realize

  widget_control, draw, $
    get_value=widget

  xmanager,'spectrum_n_see', $
    base, $
    event='spectrum_n_event',$
    group_leader=group

end
