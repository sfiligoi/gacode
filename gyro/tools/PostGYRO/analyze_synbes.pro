...
; C. Holland, UCSD
;;
; cacluates statistics for make_synbes_array output, assuming
...
; average over radius as well
  ...
  IF KEYWORD_SET(local) THEN BEGIN
      ...
      PRINT, 'RMS unfiltered n_e (%): ', gyro_ne_rms, $
             '   +/- ', gyro_ne_rms_err
      PRINT, 'RMS synthetic n_e (%): ', syn_ne_rms, $
             '   +/- ', syn_ne_rms_err
  ...
  ENDIF ELSE BEGIN
      ...
      FOR ir=0,NR_BES-1 DO BEGIN
          ...
          syn_ne_rms_err[ir] = 0.5*(INT_TABULATED(freq[idx1:idx2],$
                               ...
                               syn_ne_rms[ir]
          PRINT, 'r_min/a: ', results.syn_bes_xloc[*,0]
          print, 'delta S_syn / S_syn: ', int_Tabulated(freq[idx1:idx2],$
                                                        syn_ne_xspect_err[ir,idx1:idx2])/$
                 int_tabulated(freq[idx1:idx2],syn_ne_xspect[ir,idx1:idx2])
          PRINT, 'RMS unfiltered T_e (%): ', gyro_ne_rms[ir]*100, $
                 '   +/- ', gyro_ne_rms_err[ir]*100
          PRINT, 'RMS synthetic T_e (%): ', syn_ne_rms[ir]*100, $
   ...
   result = {$
      freq:freq,$
      gyro_ne_xspect:gyro_ne_xspect,$
      gyro_ne_xspect_err:gyro_ne_xspect_err,$
      syn_ne_xspect:syn_ne_xspect,$
      syn_ne_xspect_err:syn_ne_xspect_err,$
      gyro_ne_rms:gyro_ne_rms,$
      syn_ne_rms:syn_ne_rms,$
      gyro_ne_rms_err:gyro_ne_rms_err,$
      syn_ne_rms_err:syn_ne_rms_err  }
  ...
  IF (KEYWORD_SET(local) AND (N_ELEMENTS(file) NE 0)) THEN BEGIN
      ...
      FOR ii=0,NFFT/2 DO PRINTF, 1, freq[ii], $
        gyro_ne_xspect[ii], gyro_ne_xspect_err[ii], $
        syn_ne_xspect[ii], syn_ne_xspect_err[ii]
      CLOSE, 1
  ENDIF
END ;analyze_syncece
