...
;
; PRO Color Contour
; by C. Holland, UCSD, Dept. of Physics
; 
; procdedure to display color contour with associated intensity scale
;
; Version 2.0 updated to use keywords
; Version 3.0 updated to use filled contour plot instead of TVSCL
; Version 4.0 updated to use "Coyote" routine colorbar instead of original
;	intensity scale.  Routine is available at:
;	http://www.dfanning.com/documents/programs.html
;	It is open source.
; Version 5.0 updated to use thick keyword: set 1 for plotting in X, 2-3
;	for making PS plots.  Helps make axes much more visible for plots
;	in presentations.
; Version 6.0 Directly inserted COLORBAR routine, added xaxis, yaxis
;       parameters so that axes no longer require calls to X/Y_AXIS
; updated 6/19/07 to allow xrange, yrange keywords

PRO Color_Contour, input, xaxis, yaxis, X_AXIS = x_axis, Y_AXIS = y_axis, $
                   TITLE =  title, XTITLE = xtitle, YTITLE = ytitle, $
                   XLOG = xlog, YLOG = ylog, MIN = plotmin, MAX = plotmax, $
                   CHARSIZE = cs, NLEVELS = nlevels, LINES = lines, $
                   POSITION = full_plot_position, $
                   COLORBAR_POSITION = colorbar_position, $
                   XRANGE = xrange, YRANGE = yrange, $
                   THICK = thick, _EXTRA = extra

; keywords word as for contour
;
; updated 8/10/00 to use keywords, better logplot
; updated 8/24/00 to set fixed intensity scale
; updated 9/12/00 to be filled color contour, keywords for x/ylog
; updated 10/3/00 to fix bugs
; updated 7/5/01  change cmax to cmax - 1 to prevent black for max intensties
; updated 8/6/06 (!) b/c min/max were affecting actual data, use plot_data = input now

  plot_data = input   ;can mess around w/ plot data without affecting actual stuff

  ;make sure axes, titles initalized
  IF (N_ELEMENTS(xaxis) EQ 0) THEN IF KEYWORD_SET(x_axis) THEN $
    xaxis = x_axis ELSE xaxis = INDGEN(N_ELEMENTS(plot_data[*,0]))
  IF (N_ELEMENTS(yaxis) EQ 0) THEN IF KEYWORD_SET(y_axis) THEN $
    yaxis = y_axis ELSE yaxis = INDGEN(N_ELEMENTS(plot_data[0,*]))
;  IF NOT(KEYWORD_SET(y_axis)) THEN y_axis = INDGEN(N_ELEMENTS(plot_data[0,*]))
  IF NOT(KEYWORD_SET(title)) THEN title = ''
  IF NOT(KEYWORD_SET(xtitle)) THEN xtitle = ''
  IF NOT(KEYWORD_SET(ytitle)) THEN ytitle = ''

  ;set x/y ranges based on whether x/ylog set so we don't get errors if axes
  ;start at 0, etc.
;  IF KEYWORD_SET(xlog) THEN xrange = [xaxis[1],xaxis[N_ELEMENTS(xaxis)-1]] $
;	ELSE xrange = [xaxis[0],xaxis[N_ELEMENTS(xaxis)-1]]
;  IF KEYWORD_SET(ylog) THEN yrange = [yaxis[1],yaxis[N_ELEMENTS(yaxis)-1]] $
;	ELSE yrange = [yaxis[0],yaxis[N_ELEMENTS(yaxis)-1]]
  ;updated 6/19/07 to not enforce full range
  IF (KEYWORD_SET(xlog) AND (N_ELEMENTS(xrange) EQ 0)) THEN $
    xrange = [xaxis[1],xaxis[N_ELEMENTS(xaxis)-1]]
  IF (KEYWORD_SET(ylog) AND (N_ELEMENTS(yrange) EQ 0)) THEN $
    yrange = [yaxis[1],yaxis[N_ELEMENTS(yaxis)-1]]

  ;use n_elements instead of keword_set so plotmin, plotmax can = 0
  IF (N_ELEMENTS(plotmin) eq 0) THEN plotmin = MIN(plot_data)
  IF (N_ELEMENTS(plotmax) eq 0) THEN plotmax = MAX(plot_data)

  ;29 is maximum number of levels drawn by contour
  IF NOT(KEYWORD_SET(nlevels)) THEN nlevels = 29
  IF NOT(KEYWORD_SET(cs)) THEN cs = 1.

  ;first, pick out parts of plot_data we're interested in - set values ouside
  ;range of interest to min/max
  plot_data = plot_data > plotmin
  plot_data = plot_data < plotmax
  ...
  ;set position if not already done
  ...
  ENDIF ELSE plot_position = [0.1, 0.15, 0.8, 0.85]

  ;draw image
  IF N_ELEMENTS(thick) EQ 0 THEN thick = 1.
  CONTOUR, plot_data, xaxis, yaxis, XSTYLE = 1, YSTYLE = 1, $
           CHARSIZE = cs, TITLE = title, XTITLE = xtitle, YTITLE = ytitle, $
           /FILL, NLEVELS = nlevels, XLOG = xlog, XRANGE = xrange, $
           YLOG = ylog, YRANGE = yrange, C_COLORS = color_array, $
           POSITION = plot_position, XTHICK = thick, YTHICK = thick, $
           CHARTHICK = thick

  ;overplot lines if desired
  IF KEYWORD_SET(lines) THEN CONTOUR, plot_data, /overplot, nlevels = 29

  ;use Coyote colorbar routine, first set position
  IF NOT(KEYWORD_SET(colorbar_position)) THEN BEGIN
