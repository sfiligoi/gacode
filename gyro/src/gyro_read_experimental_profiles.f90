...
subroutine gyro_read_experimental_profiles
  ...
  dlnptotdr_p(:)     = a_meters*EXPRO_dlnptotdr(:) ! WG
  !
  dlnndr_p(:,1)  = 0.0
  dlntdr_p(:,1)  = 0.0
  dlnptotdr_p(1) = 0.0 ! WG
  !----------------------------------------------------------------------------
  ...
  do i_exp=1,n_grid_exp
     ...
     beta_unit_ptot_p(i_exp) = 400.0*ptot_exp(i_exp)/(1.6022*1e3)/(1.e5*b_unit_p(i_exp)**2) ! WG
     !-------------------------------------------------------------
