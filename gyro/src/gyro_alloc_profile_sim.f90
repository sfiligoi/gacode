...
subroutine gyro_alloc_profile_sim(flag)
  ...
  if (flag == 1) then
     ...
     allocate(beta_unit_ptot_s(n_x)) ! WG
     allocate(nu_s(n_spec,n_x))
     allocate(zmag_s(n_x))
     allocate(ptot_s(n_x)) ! WG
     allocate(w0_s(n_x))
     allocate(w0p_s(n_x))
     ...
     deallocate(beta_unit_ptot_s) ! WG
     deallocate(nu_s)
     deallocate(zmag_s)
     deallocate(ptot_s) ! WG
     deallocate(w0_s)
     deallocate(w0p_s)
