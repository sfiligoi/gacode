...
!  Takes gyro_h and generates vel_sum_p.
!------------------------------------------------------------
...
subroutine get_vel_sum_p
  ...
  implicit none
  !
  complex, dimension(n_blend,n_x) :: vel_sum_p_loc
  ...
  do p_nek=1+i_proc_1,n_nek_1,n_proc_1
     ...
     gz(:,:) = (0.0,0.0)
     do is=1,n_kinetic
        gz(:,:) = gz(:,:)+z(is)*gyro_h(:,:,p_nek_loc,is)
     enddo
     ...
     do i=1,n_x
        ...
        do m=1,n_stack
           ...
           do j=1,n_blend
              vel_sum_p_loc(j,i) = vel_sum_p_loc(j,i)+&
  ...
  !
  call MPI_ALLREDUCE(vel_sum_p_loc,&
       vel_sum_p,&
       n_x*n_blend,&
       MPI_DOUBLE_COMPLEX,&
       MPI_SUM,&
  ...
  if (n_1(in_1) == 0 .and. boundary_method == 1) then
     ...
     vel_sum_p(:,n_x) = (0.0,0.0)

  endif

  if (debug_flag == 1 .and. i_proc == 0) then
     print *,'[vel_sum_p done]'
  endif

end subroutine get_vel_sum_p
