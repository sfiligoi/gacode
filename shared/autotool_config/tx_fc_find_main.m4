#
# $Id: tx_fc_find_main.m4 3755 2010-12-07 19:34:41Z cary $
...
while test $[@%:@] != 1; do
  ...
  case $ac_arg in
    [[\\/]]*.$OBJEXT | ?:[[\\/]]*.$OBJEXT)
# Want to include this object file only if it contains the main symbol
# but the Nag fortran compiler requires linking objects not containing main
# So have to use named exclusion strategy
      case $ac_arg in
# Exclude these objects
        /var*.$OBJEXT | /tmp*.$OBJEXT | *crtbegin*.$OBJEXT | *crtend.$OBJEXT | *crtn.$OBJEXT | *crti.$OBJEXT | *crt1.$OBJEXT | *pgf90*.$OBJEXT | ifortkdYDXS.$OBJEXT | *crtsavres.$OBJEXT)
	  ;;
# Exclude these objects for pgi on freedom
        */trace_init.$OBJEXT | */initmp.$OBJEXT | */nonuma.$OBJEXT)
	  ;;
# Exclude these objects in scratch dirs
        $SCRATCH/cc*.$OBJEXT | /scratch/*.$OBJEXT | /global/scratch/*.$OBJEXT)
	  ;;
	*)
