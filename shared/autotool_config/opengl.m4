...
# Search for include files
  TX_PATH_FILE(GL_H, gl.h, "", $PATH_CHECK)
  if test -z "$GL_H"; then
    echo " "
...
# Now find library
  TX_PATH_FILES(OPENGL_LIB, libGL.dylib libGL.so libGL.a, "", $PATH_CHECK)
  if test -z "$OPENGL_LIB"; then
    ...
    TX_PATH_FILES(GLU_LIB, libGLU.dylib libGLU.so libGLU.a, "", $PATH_CHECK)
...
if test $HAVE_GL = yes;
then
  AC_DEFINE([HAVE_OPENGL], [], [description])
  AC_MSG_RESULT(yes)
fi
