...
dnl
dnl Version:	$Id: qt4.m4 3282 2009-11-22 16:00:39Z cary $
dnl
dnl Tech-X configure system
dnl
dnl ######################################################################

AC_MSG_ERROR(22Nov09: qt4.m4 is disabled.  Please move to tx_qt4.m4)
...
AM_CONDITIONAL(HAVE_GUI, test $QT_GUI = yes)
...
dnl ######################################################################
  ...
  if test -n "$QT_INCDIR"; then
    ...
    QT_INCPATH=$HOME/aix5.1/qt/include:$HOME/qt/include:/usr/local/qt/include:/loc/qt/include:/local/qt/include:/usr/lib/qt4/include:/opt/qt
  fi

  TX_PATH_FILE(QAPPLICATION_H, Qt/qapplication.h, "", $QT_INCPATH)
  if test -n "$QAPPLICATION_H"; then
      QT_INCDIR=`dirname \`dirname $QAPPLICATION_H\``
...
dnl ######################################################################
  ...
  QT_LIB="-lQtCore -lQtGui -lQt3Support -lQtOpenGL"
  AC_DEFINE(QT_THREAD_SUPPORT, , If building with QT threads.)
...
dnl ######################################################################
  ...
  TX_PATH_FILES(ABSQT_LIB, ${QT_LIBNAME}${SO} $QT_LIBNAME.a, "", $QT_LIBPATH)
...
dnl ######################################################################
  ...
  TX_PATH_FILES(JPEG_LIBDIR_PATH,libjpeg.so libjpeg.a,"",$JPEG_LIBPATH)
...
dnl ######################################################################
  ...
  TX_PATH_FILES(PNG_LIBDIR_PATH,libpng.so libpng.a,"",$PNG_LIBPATH)
...
# Determine whether X is needed
  ...
  if test "$QT_NEEDS_X11" = "yes"; then
    ...
    XMU_LIB="-lXmu"
    TX_PATH_FILES(ABS_XMULIB_PATH, libXmu.a libXmu.so, "", $X11_LIBDIR_PATH)
    ...
    XEXT_LIB="-lXext"
    TX_PATH_FILES(ABS_XEXTLIB_PATH, libXext.a libXext.so, "", $X11_LIBDIR_PATH)
    ...
    XICE_LIB="-lICE"
    TX_PATH_FILES(ABS_XICELIB_PATH, libICE.a libICE.so, "", $X11_LIBDIR_PATH)
    if test -z "$ABS_XICELIB_PATH"; then
      XICE_LIB=""
    fi

    XSM_LIB="-lSM"
    TX_PATH_FILES(ABS_XSMLIB_PATH, libSM.a libSM.so, "", $X11_LIBDIR_PATH)
    ...
    case "$host" in
      ...
      *-linux*)
       GLIDE_LIB="-lglide3"
       TX_PATH_FILE(ABS_GLIDELIB_PATH, libglide3.a, "", /usr/lib)
       if test -z "$ABS_GLIDELIB_PATH"; then
         GLIDE_LIB=""
       fi
       XPM_LIB="-lXpm"
       TX_PATH_FILES(ABS_XPMLIB_PATH, libXpm.a libXpm.so, "", $X11_LIBDIR_PATH)
       if test -z "$ABS_XPMLIB_PATH"; then
         XPM_LIB=""
       fi
       XFT_LIB="-lXft"
       TX_PATH_FILES(ABS_XFTLIB_PATH, libXft.a libXft.so, "", $X11_LIBDIR_PATH)
       if test -z "$ABS_XFTLIB_PATH"; then
         XFT_LIB=""
       fi
       XFREETYPE_LIB="-lfreetype"
       TX_PATH_FILES(ABS_XFREETYPELIB_PATH, libfreetype.a libfreetype.so, "", /usr/lib)
       if test -z "$ABS_XFREETYPELIB_PATH"; then
         XFREETYPE_LIB=""
       fi
       XRENDER_LIB="-lXrender"
       TX_PATH_FILES(ABS_XRENDERLIB_PATH, libXrender.a libXrender.so, "", $X11_LIBDIR_PATH)
       if test -z "$ABS_XRENDERLIB_PATH"; then
         XRENDER_LIB=""
       fi
       XINERAMA_LIB="-lXinerama"
       TX_PATH_FILES(ABS_XINERAMALIB_PATH, libXinerama.a libXinerama.so, "", $X11_LIBDIR_PATH)
