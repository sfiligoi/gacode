...
dnl
dnl Version:	$Id: rsiidl.m4 2788 2008-10-31 01:37:01Z sizemore $
dnl
...
# if this test succeeds then using version 5.6 or greater
  ...
  TX_PATH_FILE(RSIIDL_HASINC, idl_export.h, "", $RSIIDL_INCDIR)
...
# This is for version 5.5 or earlier
  if test -z "$RSIIDL_HASINC"; then
    ...
    TX_PATH_FILE(RSIIDL_HASINC, export.h, "", $RSIIDL_INCDIR)
    if test -z "$RSIIDL_HASINC"; then
...
RSIIDL_LIBBIN="$withval", RSIIDL_LIBBIN="$IDL_LIBBIN")


  if test -z "$RSIIDL_LIBBIN"; then
     AC_MSG_ERROR(Can't find RSI-IDL libidl.so on this platform; searched $RSIIDL_LIBBIN)
  fi
