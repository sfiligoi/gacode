...
dnl
dnl Version:    $Id: fc.m4 3400 2010-02-18 19:05:06Z pletzer $
dnl
...
dnl Look for specified compiler or else any of those supported
    if test -n "$FC"; then
      ...
      FCS="lf95 pgf90 pathf90 xlf90 gfortran gfortran4 ifort ifc f90 "
    fi
...
dnl ----------------------------------------------------------------------
dnl FCBASE is in case the FC flag gets overwritten by mpif90
dnl ----------------------------------------------------------------------
if test -n "$FC"; then
  if test -z "$SERIAL_F90"; then
    SERIAL_F90=`basename $FC`
  fi
fi
AC_SUBST(SERIAL_F90)
