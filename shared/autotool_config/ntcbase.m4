...
dnl
dnl Version:	$Id: ntcbase.m4 3366 2010-01-15 18:43:10Z dws $
dnl
...
dnl builtin(include,config/jcorba.m4)
...
dnl ######################################################################
dnl 
dnl preffile=$HOME/.netscape/preferences.js
dnl echo $ac_n "Adding necessary lines to $preffile... " $ac_c
dnl if test ! -f $preffile; then
dnl   echo $preffile not found!
dnl else
dnl   hasline=`grep signed.applets.local_classes_have_30_powers $preffile`
dnl   if test -z "$hasline"; then
dnl     netscaperunning=`ps -u $USER 2>/dev/null | grep netscape`
dnl     if test -n "$netscaperunning"; then
dnl       "cannot because netscape is running.  Quit netscape and redo if you need to add necessary lines to netscape's preferences."
dnl     else
dnl       echo 'user_pref("signed.applets.codebase_principal_support", true);' \
dnl         >> $preffile
dnl       echo 'user_pref("signed.applets.local_classes_have_30_powers", true);' \
dnl         >> $preffile
dnl       echo 'user_pref("signed.applets.low_security_for_local_classes", true);' \
dnl         >> $preffile
dnl       echo done.
dnl     fi
dnl   else
dnl     echo lines already present.
dnl   fi
dnl fi
